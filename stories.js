/**
 * ── STORY BOARD DATA ──
 * Single source of truth for all story board entries.
 * Add new stories here — both the homepage and storyboard page
 * will automatically pick them up and sort by date (newest first).
 *
 * Fields:
 *   date   – ISO date string (YYYY-MM-DD) used for sorting
 *   label  – human-readable date shown on the card
 *   title  – company / project name
 *   desc   – short description
 *   img    – path to image in /images/storyboard/
 *   alt    – image alt text
 */
 ...
 * and return the requested count (or all).
 */
export function getSortedStories(limit) {
 ...
 * @param {number|null} limit    – max cards to render (null = all)
 */
export function renderStoryCards(gridSelector, limit = null) {
  const grid = document.querySelector(gridSelector);
  if (!grid) return;

  const items = getSortedStories(limit);
